...
$WindowsProject = Join-Path $RepoRoot 'src\windows\Bob.Windows\Bob.Windows.csproj'
...
try {
    if ($Target -in @('All', 'Windows')) {
        Assert-File -Path $WindowsProject -Label 'Windows project'
        Assert-File -Path $NuGetConfig -Label 'Repository NuGet config'
        & $DotnetPath restore $WindowsProject --configfile $NuGetConfig --nologo
        Assert-LastExitCode -Action 'Windows restore'
        & $DotnetPath build $WindowsProject --configuration $Configuration --no-restore --nologo
        Assert-LastExitCode -Action 'Windows build'
    }
    ...
    if ($Target -in @('All', 'Android')) {
        ...
        & $GradlePath --project-dir $AndroidProject --console=plain $gradleTask
        Assert-LastExitCode -Action 'Android build'
    }
}
finally {
    Pop-Location
